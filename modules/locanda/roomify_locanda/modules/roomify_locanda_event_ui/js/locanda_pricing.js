(function ($) {
// define object
Drupal.LocandaPricing = Drupal.LocandaPricing || {};
Drupal.LocandaPricing.Modal = Drupal.LocandaPricing.Modal || {};
...
$(document).ready(function () {
  $(document).on('click', '.fc-resource-area tr', function () {
    ...
    if (resourceId != undefined && resourceId.match(/^S[0-9]+$/)) {
      Drupal.LocandaPricing.Modal(this, resourceId.replace('S', ''));
    }
  });
...
Drupal.behaviors.locanda_pricing = {
 ...
 */
Drupal.LocandaPricing.Modal = function(element, $resource_id) {
  Drupal.CTools.Modal.show('bat-modal-style');

  // The base url (which doesn't change) is used to identify our ajax instance.
  var base = Drupal.settings.basePath + '?q=admin/locanda/rates/manage/';
