...
function roomify_elegant_status_messages($variables) {
  if (module_exists('better_messages')) {
    return theme_better_messages($variables);
  }
}

function rroomify_elegant_current_search_item_wrapper(array $variables) {
...
function roomify_elegant_preprocess_slick_item(&$variables) {
 ...
 */
function roomify_elegant_field__field_listing_enquiry_dep_date($variables) {
  $output = '';
 ...
 * Implements theme_field().
 */
function rroomify_elegant_field__field_listing_enquiry_type_ref($variables) {
  $output = '';
 ...
 * Implements theme_field().
 */
function roomify_elegant_field__field_listing_enquiry_guests($variables) {
  $output = '';
 ...
 * Implements theme_field().
 */
function roomify_elegant_field__field_listing_enquiry_arr_date($variables) {
  $output = '';
 ...
 * Implements theme_field().
 */
function roomify_elegant_field__field_listing_enquiry_name($variables) {
  $output = '';
 ...
 * Implements theme_field().
 */
function roomify_elegant_field__field_listing_enquiry_email($variables) {
  $output = '';
 ...
 * Implements theme_field().
 */
function roomify_elegant_field__field_listing_enquiry_info($variables) {
  $output = '';
 ...
 */
function roomify_elegant_preprocess_html(&$variables) {
 ...
 */
function roomify_elegant_process_page(&$variables) {
 ...
 */
function roomify_elegant_process_html(&$vars) {
  // Hook into color.module
  if (module_exists('color')) {
 ...
 */
function roomify_elegant_search_api_sorts_sort(array $variables) {
  $name = $variables['name'];
  $path = $variables['path'];
  ...
  elseif ($variables['active']) {
    $return_html = '<span class="search-api-sort-active">';
    $return_html .= l(t($name) . _roomify_elegant_theme_sort_indicator($order_options['query']['order']), $path, $order_options);
    $return_html .= '</span>';
  }
  else {
 ...
 */
function _roomify_elegant_theme_sort_indicator($style) {
  if ($style == 'asc') {
    $asc = drupal_get_path('theme', 'roomify_elegant') . '/images/sort-asc.svg';
    return file_get_contents($asc);
  }
  else {
    $desc = drupal_get_path('theme', 'roomify_elegant') . '/images/sort-desc.svg';
 ...
 */
function roomify_elegant_preprocess_mimemail_message(&$variables) {
  $theme = variable_get('theme_default', 'roomify_elegant');
 ...
 */
function roomify_elegant_preprocess_page(&$variables) {
